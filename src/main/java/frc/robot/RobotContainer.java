...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        switch (RobotConstants.Instance().kMode) {
            ...
            case SIM:
                ...
                kIndexer = new Indexer(new IndexerIOSim());
                break;
            default:
                break;
        }
    ...
    private void configureBindings() {
        ...
        kDriveController.a().onTrue(  
            kIndexer.setIndexerStateCommand(Indexer.IndexerState.Active)
        );

        kDriveController.b().onTrue(
            kIndexer.setIndexerStateCommand(Indexer.IndexerState.Idle)
        );

        kDriveController.x().onTrue(
            kIndexer.setIndexerStateCommand(Indexer.IndexerState.Eject)
        );
    }
}
